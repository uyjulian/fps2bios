...
int EnableIntr(int interrupt)
{
	...
	{
		DMA_ICR = (DMA_ICR & ((~(1 << (low_irq - INUM_DMA_0))) & 0x00FFFFFF)) | (1 << (INUM_DMA_BERR - INUM_DMA_0 + 16)) | (1 << (low_irq - INUM_DMA_0 + 0x10)) | ((high_irq & IMODE_DMA_IRM) ? (1 << (low_irq - INUM_DMA_0)) : 0);

		DMA_ICR2 = (DMA_ICR2 & ((~(1 << (low_irq - INUM_DMA_0))) & 0x00FFFFFF)) | ((high_irq & IMODE_DMA_IQE) ? (1 << (low_irq - 0x10)) : 0);

		IMASK |= 1 << INUM_DMA;
	}
	else if ((low_irq - INUM_DMA_7) < 6)
	{ // low_irq = 0x27 isn't handled?
...
int DisableIntr(int interrupt, int* oldstat)
{
	...
	if (low_irq < 0x27)
	{
		...
		{
			if (((DMA_ICR & 0xFFFFFF) >> (low_irq - 0x20)) & 0x01)
				stat = low_irq | 0x100;

			if (DMA_ICR2 & (1 << (low_irq - 0x20)))
				stat |= 0x200;
	...
	if (low_irq < 0x2E)
	{
		if (DMA_ICR2 & 0xFFFFFF & (1 << (low_irq - 0x18)))
		{
			...
			if (((DMA_ICR2 & 0xFFFFFF) >> (low_irq - 0x21)) & 0x01)
				stat |= 0x200;
