...
int link_client(struct import* imp)
{
	...
	for (e = lc_internals.let_next; e; e = (struct export*)e->magic_link)
	{
		if (debug > 0)
		{
			...
			*(int*)ename = *(int*)e->name;
			*(int*)(ename + 4) = *(int*)(e->name + 4);
			ename[8] = 0;
			*(int*)iname = *(int*)imp->name;
			*(int*)(iname + 4) = *(int*)(imp->name + 4);
			iname[8] = 0;
...
int RegisterLibraryEntries(struct export* es)
{
	...
	{
		for (tmp = lc_internals.mda_next; tmp->next; tmp = tmp->next)
		{ //free
