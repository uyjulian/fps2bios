...
extern void* _end;
void* sysmem_init_memory()
{ //mark all mem as not allocated, available
	unsigned int i;
	struct allocELEM* p;
	...
	{
		if (alloclist == AllocSysMemory(ALLOC_FIRST, sizeof(struct allocTABLE) - 4, NULL))
		{ //alloc allocated allocation table;-)
...
void* alloc(int flags, int size, void* mem)
{
	...
	switch (flags)
	{
		...
		case ALLOC_LATER:
			if ((unsigned int)mem & 0xFF)
				return NULL;
			baddress = (unsigned int)mem >> 8; //addr in 256bytes units
			for (a = alloclist->list; a; a = a->next)
			{
				if (baddress < mADDRESS(a->info))
					return NULL; //cannot realloc
				if ((!mALLOCATED(a->info)) &&
...
int free(void* mem)
{
	...
	if ((unsigned int)mem & 0xFF)
		return -1; // only 256byte multiple
	...
	if ((a->next) && (!mALLOCATED(a->next->info)))
	{ //bind with next free block
		n = a->next;
		skip = 1;
	...
	if (p && (!mALLOCATED(p->info)))
	{ //bind with previous free block
		n = a;
		skip++; // or skip=2;
...
///////////////////////////////////////////////////////////////////////[OK]
struct allocELEM* findblock(void* a)
{ // it finds the block for a given address
	struct allocELEM* p;
	for (p = alloclist->list; p; p = p->next)
